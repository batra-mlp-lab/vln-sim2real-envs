...
var camera, camera_pose, scene, controls, renderer, skybox, raycaster, connections, world_frame, cylinder_frame, cubemap_frame;
...
function load_cube(scan, image) {
  ...
  //var urlPrefix = "/matterport_skybox_images/" + image;
  ...
  loader.load(urls, function(textureCube){
    ...
    //var cam_pose = cylinder_frame.getObjectByName(image);
    //set_camera_pose(cam_pose.matrix, cam_pose.height);

    var cam_pose = cylinder_frame.getObjectByName(image);
    cylinder_frame.position.x = -cam_pose.position.x;
    cylinder_frame.position.y = -cam_pose.position.y;
    cylinder_frame.position.z = -cam_pose.position.z;
        
    // Adjust cylinder visibility
    var cylinders = cylinder_frame.children;
    for (var i = 0; i < cylinders.length; ++i){
      cylinders[i].visible = connections[image]['unobstructed'][i] && cylinders[i].included;
    }
...
function load_connections(scan) {
  ...
  //var url	= DATA_DIR+scan+"_pose_visibility_checked_v1.json";
	var url	= "data/v1/scans/" +scan+ "/matterport_camera_poses/pose_visibility.json";
